...
MSCCLPP_DEVICE_INLINE void
  ...
  int sent_local; // used only if is_first_block and send_proxy
  if (is_first_block) {
    if (send_sm) {
      if (!recv_sm && !recv_proxy && send_sm && tid == 0) send_sm_channel->signal(nloops);
      sent_local = nloops;
    } else {
      sent_local = (send_proxy ? 0 : nloops);
    }
  } else {
    sent_local = nloops;
  }
  ...
  while (reduced < nloops || sent_local < nloops) {
    ...
    if (reduced < received) {
      ...
      if (recv_sm && nrecv_peers == 1) {
        ...
        // no __threadfence() needed, only one threadblock is writing (nrecv_peers == 1)
      } else {
        // Try lock
    ...
    if (send_proxy && sent_local < reduced) {
