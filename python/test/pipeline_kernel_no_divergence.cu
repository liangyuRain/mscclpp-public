...
MSCCLPP_DEVICE_INLINE void 
  ...
  assert(nrecv_sm <= warpSize);
  assert(nsend_sm <= warpSize);
  assert(nrecv_proxy <= warpSize);
  assert(nsend_proxy <= warpSize);
      ...
      if (rloop < nloops) {
        ...
        if (warp_idx == 0) {
          ...
          if (i < nrecv_sm) {
            ...
            // if (ready_loop < rloop + 1) ready[i] = ready_loop + recv_sm_channels[i].poll(rloop + 1 - ready_loop);
            if (ready_loop < nloops) ready[i] = ready_loop + recv_sm_channels[i].poll(nloops - ready_loop);
          }
        } else if (warp_idx == 1) {
          const int i = tid - warpSize;
          const int j = i + nrecv_sm;
          if (i < nrecv_proxy) {
            const int ready_loop = ready[j];
            // if (ready_loop < rloop + 1) ready[j] = ready_loop + recv_proxy_channels[i].poll(rloop + 1 - ready_loop);
            if (ready_loop < nloops) ready[j] = ready_loop + recv_proxy_channels[i].poll(nloops - ready_loop);
          }
        }
        __syncthreads();
        ...
        if (warp_idx == 0) {
          const int i = tid;
          if (i < nrecv_sm && count[i] > 0) recv_sm_channels[i].signal(count[i]);
        } else if (warp_idx == 1) {
          const int i = tid - warpSize;
          const int j = i + nrecv_sm;
          if (i < nrecv_proxy && count[j] > 0) {
            const int before = reduced[j] - count[j];
            if (before > 0 && before / FLUSH_INTERVAL < reduced[j] / FLUSH_INTERVAL) recv_proxy_channels[i].flush();
            recv_proxy_channels[i].signal(count[j]);
          }
        }
