...
@pytest.mark.parametrize("nelem_per_send", [8])
@pytest.mark.parametrize("nelem_total", [20])
@pytest.mark.parametrize("scratch_size", [20])
@pytest.mark.parametrize("sm_node_size", [1])
def test_RS_kernel_send_recv_reduce_chain(mpi_group: MpiGroup, nelem_per_send: int, nelem_total: int,
                                          scratch_size: int, sm_node_size: int):
    if mpi_group.comm.rank == 0:
        print(f"TEST: test_RS_kernel_send_recv_reduce_chain("
              f"mpi_group.comm.size={mpi_group.comm.size}, "
              f"nelem_per_send={nelem_per_send}, "
              f"nelem_total={nelem_total}, "
              f"scratch_size={scratch_size}, "
