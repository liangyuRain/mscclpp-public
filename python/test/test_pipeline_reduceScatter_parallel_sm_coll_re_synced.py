...
from .pipeline_schedule import (
    ReduceScatterParallelSMCollREPipelineKernel,
...
@pytest.mark.parametrize("nelem_total", [1024])
@pytest.mark.parametrize("scratch_size", [20, 32])
@pytest.mark.parametrize("sm_node_size", [1, 4, 8])
@pytest.mark.parametrize("n_parallel_sm_blocks", [1, 4])
@pytest.mark.parametrize("n_parallel_reduce_blocks", [1, 4])
@pytest.mark.parametrize("n_pipeline", [1, 2, 4])
...
def test_send_recv_reduce_chain(mpi_group: MpiGroup, nelem_per_send: int, nelem_total: int,
                                ...
                                n_parallel_reduce_blocks: int, n_pipeline: int, skip_leaf_tb: bool):
    ...
    data_chunk_offsets, data_chunk_sizes = {}, {}
    pipeline_groups = [list(range(n_pipeline))]
    for pp in range(n_pipeline):
        if group.my_rank == 0:
            ...
            if group.my_rank // sm_node_size == dest // sm_node_size:
                send_sm_channels[pp] = [group.make_sm_channel(memory, connections[dest], dest)]
            else:
                send_proxy_channels[pp] = [group.make_proxy_channel(proxy_service, memory, connections[dest], dest)]
            ...
            if group.my_rank // sm_node_size == src // sm_node_size:
                recv_sm_channels[pp] = [group.make_sm_channel(scratch, connections[src], src)]
                recv_sm_scratches[pp] = [scratch]
            else:
                recv_proxy_channels[pp] = [group.make_proxy_channel(proxy_service, scratch, connections[src], src)]
                recv_proxy_scratches[pp] = [scratch]
        else:
            # recv reduce send
            scratch = cp.array([1000] * scratch_size, dtype=cp.int32)
            src = group.my_rank - 1
            ...
            if group.my_rank // sm_node_size == dest // sm_node_size:
                ...
                send_proxy_channels[pp] = [group.make_proxy_channel(proxy_service, memory, connections[dest], dest)]
        data_chunk_offsets[pp] = pp
        data_chunk_sizes[pp] = 1
    total_chunks = n_pipeline
    ntrees = n_pipeline
    ...
    if group.my_rank == 0 and skip_leaf_tb and group.my_rank // sm_node_size == dest // sm_node_size:
        ...
        if skip_leaf_tb:
            leaf_nodes = {pp: [] if src is None else [src == 0] for pp in range(n_pipeline)}
        else:
            leaf_nodes = None
        kernel = ReduceScatterParallelSMCollREPipelineKernel(recv_sm_channels, send_sm_channels, recv_proxy_channels, send_proxy_channels,
                                                             ...
                                                             n_parallel_reduce_blocks, leaf_nodes, skip_leaf_tb, pipeline_groups)
        proxy_service.start_proxy()
        group.barrier()
...
@pytest.mark.parametrize("scratch_size", [20, 32])
@pytest.mark.parametrize("sm_node_size", [1, 4, 8])
@pytest.mark.parametrize("n_parallel_sm_blocks", [1, 2])
@pytest.mark.parametrize("n_parallel_reduce_blocks", [1, 2])
@pytest.mark.parametrize("n_pipeline", [1, 2, 4])
@pytest.mark.parametrize("skip_leaf_tb", [False, True])
def test_multipeer_reduce(mpi_group: MpiGroup, nelem_per_send: int, nelem_total: int, scratch_size: int,
                          sm_node_size: int, n_parallel_sm_blocks: int, n_parallel_reduce_blocks: int,
                          n_pipeline:int, skip_leaf_tb: bool):
    if mpi_group.comm.rank == 0:
    ...
    for pp in range(n_pipeline):
        if group.my_rank == 0:
            ...
            recv_sm_scratches[pp] = [cp.array([1000] * scratch_size, dtype=cp.int32) for _ in sm_recv_peers]
            recv_sm_channels[pp] = [group.make_sm_channel(recv_sm_scratches[pp][idx], connections[dest], dest)
            ...
            recv_proxy_scratches[pp] = [cp.array([1000] * scratch_size, dtype=cp.int32) for _ in proxy_recv_peers]
            recv_proxy_channels[pp] = [group.make_proxy_channel(proxy_service, recv_proxy_scratches[pp][idx], connections[dest], dest)
                                       for idx, dest in enumerate(proxy_recv_peers)]
        else:
            # sender
            if group.my_rank // sm_node_size == 0 // sm_node_size:
                send_sm_channels[pp] = [group.make_sm_channel(memory, connections[0], 0)]
            else:
                send_proxy_channels[pp] = [group.make_proxy_channel(proxy_service, memory, connections[0], 0)]
        data_chunk_offsets[pp] = pp
        data_chunk_sizes[pp] = 1
    total_chunks = n_pipeline
    ntrees = n_pipeline
    ...
    if group.my_rank != 0 and skip_leaf_tb and group.my_rank // sm_node_size == 0 // sm_node_size:
        ...
        if skip_leaf_tb:
            if group.my_rank == 0:
                leaf_nodes = {pp: [True for _ in range(1, group.nranks)] for pp in range(n_pipeline)}
            else:
                leaf_nodes = {pp: [] for pp in range(n_pipeline)}
        else:
            leaf_nodes = None
        kernel = ReduceScatterParallelSMCollREPipelineKernel(recv_sm_channels, send_sm_channels, recv_proxy_channels, send_proxy_channels,
                                                             memory, data_chunk_offsets, data_chunk_sizes, total_chunks, scratch_size,
...
def test_tree_reduce_to_root(nelem_per_send: int, nelem_total: int, scratch_size: int, sm_node_size: int,
                             n_parallel_sm_blocks: int, n_parallel_reduce_blocks: int, n_pipeline: int,
                             skip_leaf_tb: bool):
    ...
    if group.my_rank in [4, 5, 6, 7] and skip_leaf_tb and group.my_rank // sm_node_size == send_peers[0] // sm_node_size:
        ...
        if skip_leaf_tb:
            leaf_nodes = {pp: [src in [4, 5, 6, 7] for src in recv_peers] for pp in range(n_pipeline)}
        else:
            leaf_nodes = None
        kernel = ReduceScatterParallelSMCollREPipelineKernel(recv_sm_channels, send_sm_channels, recv_proxy_channels, send_proxy_channels,
                                                             memory, data_chunk_offsets, data_chunk_sizes, total_chunks, scratch_size,
...
@parametrize_mpi_groups(2, 8)
@pytest.mark.parametrize("nelem_per_send", [8, 20])
@pytest.mark.parametrize("reduce_scatter_length", [2 ** 20])
@pytest.mark.parametrize("scratch_size", [20, 32])
@pytest.mark.parametrize("ninstance", [1, 2])
@pytest.mark.parametrize("sm_node_size", [1, 4, 8])
@pytest.mark.parametrize("n_parallel_sm_blocks", [1, 2])
@pytest.mark.parametrize("n_parallel_reduce_blocks", [1, 2])
@pytest.mark.parametrize("n_pipeline", [2, 4])
@pytest.mark.parametrize("skip_leaf_tb", [False, True])
def test_allpair_reduce_scatter(mpi_group: MpiGroup, nelem_per_send: int, reduce_scatter_length: int,
                                ...
                                n_parallel_sm_blocks: int, n_parallel_reduce_blocks: int,
                                n_pipeline: int, skip_leaf_tb: bool):
    if mpi_group.comm.rank == 0:
    ...
    try:
        ...
        print(f"Skip ThreadBlockLimitException: {e}", flush=True)
        return
...
@pytest.mark.parametrize("reduce_scatter_length", [3 * 2 ** 20])
...
def test_multrun_reduce_scatter(nelem_per_send: int, reduce_scatter_length: int, scratch_size: int,
                                iters: int, sm_node_size: int, n_parallel_sm_blocks: int,
                                n_parallel_reduce_blocks: int, n_pipeline: int, skip_leaf_tb: bool):
    if n_pipeline == 4 and not skip_leaf_tb:
        return
    mpi_group = MpiGroup(list(range(8)))
    if mpi_group.comm.rank == 0:
