...
- Currently, the best reduce-scatter performance is achieved by `pipeline_reduceScatter_kernel_parallel_sm_sendtb.cu` at large sizes and `pipeline_reduceScatter_kernel_parallel_sm_coll_re_synced.cu` at small sizes. For allgather, `pipeline_allgather_kernel_parallel_sm.cu` has the best performance. One can also check the expt result files, as they specify the kernel used in the runs.
